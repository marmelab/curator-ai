...
export const getUnwantedThemes = async (mail: string) => {
    ...
    if (themesError) {
        throw new Error(`Error retrieving unwanted themes: ${themesError.message}`);
    }

    console.log('Retrieved unwanted themes:', themesData?.themes);
    return themesData;
}
...
export const addThemes = async (themes: string[], unwanted_themes: string[], mail: string) => {
    const oldThemes = await getThemes(mail);
    const oldUnwantedThemes = await getUnwantedThemes(mail);
    const newThemes = arrayUnion(arrayDifference(oldThemes?.themes || [], unwanted_themes), themes);
    const newUnwantedThemes = arrayUnion(arrayDifference(oldUnwantedThemes?.themes || [], themes), unwanted_themes);
