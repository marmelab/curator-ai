...
async function runNewsletter() {
  ...
  // Sending email :
  if (process.env.POSTMARK_SERVER_API_KEY && process.env.POSTMARK_DEFAULT_MAIL && send_mail) {

    const client = new ServerClient(process.env.POSTMARK_SERVER_API_KEY as string);
    ...
    if (!(process.env.POSTMARK_SERVER_API_KEY) || !(process.env.POSTMARK_DEFAULT_MAIL)) {
      console.error('Make sure to define POSTMARK_SERVER_API_KEY and POSTMARK_DEFAULT_MAIL if you want to send mail');
    }
